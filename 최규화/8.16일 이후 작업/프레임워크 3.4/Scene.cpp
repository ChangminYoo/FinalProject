...
void Scene::SET_DRAW_STATE_BY_DEATH_BY_SERVER_DATA(const unsigned short& id, const STC_DrawState & data)
{
	for (auto& GameObject : DynamicObject)
	{
		...
		if (id == GameObject->m_player_data.id)
		{
			...
			GameObject->m_player_data.alive = true;
			
			Player->m_async_client->m_start_attack = false;
			GameObject->m_end_attack = true;
...
void Scene::SET_END_BULLET_BY_SERVER_DATA(const STC_BulletObject_EndPos_Info & ebulldata)
{
	//이동 전 첫번째 불렛이다 - 이 때만 Create로 불렛 생성
	if (ebulldata.master_id != Player->PlayerObject->m_player_data.id)
	{
		XMFLOAT3 xmf3_pos = { ebulldata.lookvector.x , ebulldata.lookvector.y, ebulldata.lookvector.z };
		if (ebulldata.type == protocol_DiceBullet)
			Player->CreateOtherClientEndDicestrikeSkill(device, commandlist, ebulldata.endpoint, nullptr, &BulletObject, ebulldata, xmf3_pos);
		else
			Player->CreateOtherClientEndBullet(device, commandlist, ebulldata.endpoint, nullptr, &BulletObject, ebulldata);
...
{
	for (const auto& GameObject : DynamicObject)
	{
		if (GameObject->m_player_data.id == id)
		{
			delete GameObject;
...
void Scene::SET_PLAYER_SKILL(const STC_HammerSkillInfo & hammer_bullet)
{
		...
		if (hmdata.create_first)
		{
			for (const auto& client : DynamicObject)
			{
				...
				if (client->m_player_data.id == hmdata.master_id)
				{
					...
					cout << "MyClient to OtherClient HammerBullet Created\n";
				}

				cout << "Client ID: " << static_cast<int>(client->m_player_data.id) << "\n";
			}
		}
	}
	else 
	{
		...
		//내 클라이언트가 쏜 해머불렛에 상대가 맞아서 상대 주변에 서버에서 불렛을 추가시켜준 상태, 이를 클라에서 따로 또 객체생성을 해서 보여줘야한다
		if (hmdata.create_first == true && hmdata.headBullet == false)
		{
			...
			cout << "Ohter hammerbullet is created at server\n";
		}
	}

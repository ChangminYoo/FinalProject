...
void Scene::CreateRootSignature()
{
	...
	//루트시그니처의 정보구조체 생성
	CD3DX12_ROOT_SIGNATURE_DESC rootSigDesc(4, RootParameter, (UINT)staticSamplers.size(), staticSamplers.data(),
...
Scene::Scene(ID3D12Device * m_Device, ID3D12GraphicsCommandList * m_DC)
{
	...
	camera = new CCamera(m_Device, m_DC, 800, 600);
	...
	testobj[0] = new CZombieObject(m_Device, m_DC);
	testobj[1] = new CZombieObject(m_Device, m_DC, XMFLOAT4(35, 0, -23, 0));
	testobj[2] = new CZombieObject(m_Device, m_DC, XMFLOAT4(-35, 0, 25, 0));
	
	Shaders[0]->testObj[0] = testobj[0];
	Shaders[0]->testObj[1] = testobj[1];
	Shaders[0]->testObj[2] = testobj[2];
...
void Scene::Render()
{
	...
	if (nShader > 0)
	{
		for (int i = 0; i < nShader; i++)
		{
			...
			camera->UpdateConstantBuffer(commandlist);

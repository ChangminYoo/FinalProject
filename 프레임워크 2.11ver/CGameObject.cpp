...
CCubeManObject::CCubeManObject(ID3D12Device * m_Device, ID3D12GraphicsCommandList * commandlist, XMFLOAT4 cp) : CGameObject(m_Device,commandlist,cp)
{
	...
	if (CreateMesh == false)
	{
		...
		LoadTexture(m_Device, commandlist,this, Textures,SrvDescriptorHeap,"WizardTex", L"textures/human/Male White Wizard 05 Red.dds");
	...
	pp->SetHalfBox(4, 8.5, 4);//충돌 박스의 x,y,z 크기
...
void CCubeManObject::SetMesh(ID3D12Device * m_Device, ID3D12GraphicsCommandList* commandlist)
{
	...
	//애니메이션 로드
	LoadMD5Anim(m_Device, L".\\플레이어메쉬들\\crying.MD5ANIM",&Mesh, this, animations);//0
	...
	LoadMD5Anim(m_Device, L".\\플레이어메쉬들\\crying.MD5ANIM", &Mesh, this, animations);//3
	LoadMD5Anim(m_Device, L".\\플레이어메쉬들\\idle.MD5ANIM", &Mesh, this, animations);//4

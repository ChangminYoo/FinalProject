...
//	Player *player;
	std::vector<RenderItem*> playerItems;
...
void TexColumnsApp::Update(const GameTimer& gt)
{
	...
	//PlayerRender(mCommandList.Get(), playerItems);
}
...
void TexColumnsApp::LoadTextures()
{
	...
	mTextures[fbxTex->Name] = std::move(fbxTex);
...
void TexColumnsApp::BuildDescriptorHeaps()
{
	...
	auto fbxTex = mTextures["fbxTex"]->Resource;
...
void TexColumnsApp::BuildMaterials()
{
	...
	auto fbx0 = std::make_unique<Material>();
	fbx0->Name = "fbx0";
	fbx0->MatCBIndex = 3;
...
void TexColumnsApp::BuildRenderItems()
{
	...
	fbxRitem->Mat = mMaterials["fbx0"].get();

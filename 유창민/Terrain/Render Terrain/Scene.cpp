	...
	m_ResMgr(DEV, FRAME_BUFFER_COUNT, 6, 23, 0), m_Cam(height, width), m_DNC(6000, 4096)
{
	...
	m_pT = new Terrain(&m_ResMgr, new TerrainMaterial(&m_ResMgr, "grassnormals.png", "snownormals.png",
		...
		"rockdiffuse.png", colors), "heightmap8.png", "displacement.png");
	...
	// 2D 및 3D 렌더링을위한 그래픽 파이프 라인을 초기화.
...
void Scene::DrawShadowMap(ID3D12GraphicsCommandList* cmdList)
{
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		// mDrawMode : 0/false = 2D rendering , 1/true = 3D rendering
...
void Scene::BuildShapeGeometry()
{
	...
	{
		vertices[0].position = box.Vertices[i].Position;
		vertices[0].normal = box.Vertices[i].Normal;
	}
	...
	auto iBuffer = m_ResMgr.NewBuffer(buffer, &CD3DX12_RESOURCE_DESC::Buffer(VertexCount * sizeof(Vertex)),
		&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE,
		D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, nullptr);
	buffer->SetName(L"Box Vertex Buffer");
	auto sizeofVertexBuffer = GetRequiredIntermediateSize(buffer, 0, 1);


	D3D12_SUBRESOURCE_DATA dataVB = {};
	dataVB.pData = vertices.data();
	dataVB.RowPitch = sizeofVertexBuffer;
	dataVB.SlicePitch = sizeofVertexBuffer;

	m_ResMgr.UploadToBuffer(iBuffer, 1, &dataVB, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);

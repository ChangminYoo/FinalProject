...
void CPlayer::CreateBullet(ID3D12Device* Device, ID3D12GraphicsCommandList* cl,XMFLOAT3 & Goal,CGameObject* lock, list<CGameObject*>* bulletlist)
{
	...
	case 3://다이스트라이크
	{
		...
		//PlayerObject->ParticleList->push_back(new  BarObject(Device, cl, PlayerObject->ParticleList, PlayerObject, 10, XMFLOAT4(PlayerObject->CenterPos.x, 30, PlayerObject->CenterPos.z, 0)));
		PlayerObject->ParticleList->push_back(new  DiceObject(Device, cl, PlayerObject->ParticleList, PlayerObject,  XMFLOAT4(PlayerObject->CenterPos.x, 15, PlayerObject->CenterPos.z, 0)));
		auto v = Float3Add(Goal, XMFloat4to3(PlayerObject->CenterPos), false);
